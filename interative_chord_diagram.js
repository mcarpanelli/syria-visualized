...
var matrix_raw, matrix, NameSource, NameSource_Short;
    ...
    queue()
        ...
        .await(function(error, data1){
            if (!error) {
                ...
                matrix_raw.forEach(function(d){
                    d.events = parseFloat(d.events);
                ...
                //console.log(matrix_raw);

                NameSource = ["Government/Military","Civilians/Protesters","Foreign","Rebels","ISIS","Al Qaeda / Jabhat al-Nusra","Kurds","Unknown/Other"];
                NameSource_Short = ["Government/Military","Civilians/Protesters","Foreign","Rebels","ISIS","A/J","Kurds","Other"];

                matrix = [];
                for (i = 0; i < NameSource.length; i++){
                    matrix[i] = [];
                    for (j = 0; j < NameSource.length; j++){
                        if (matrix_raw.filter(function(d){return d.Source_Sector_General == NameSource[i] && d.Target_Sector_General == NameSource[j]}).length > 0)
                        {
                            var help = matrix_raw.filter(function(d){return d.Source_Sector_General == NameSource[i] && d.Target_Sector_General == NameSource[j]})[0]["events"];
                            matrix[i].push(help);
                        }
...
function createVisualizations(){
    ...
    function Draw1(){
        ...
        changeTopText(newText = "The next steps show the spread of violence, using data on "+d3.format(',d')(eventdata.length)+" events reported in the media in 2011-2015.",
    ...
    function Draw2(){
        ...
        /*Switch  text*/
        changeTopText(newText = Math.floor(matrix_raw.filter(function(d){return d.Source_Sector_General == NameSource[0];}).sum("events"))+"% of reported violent events include the Government / Military as a source.",
    ...
    function Draw3(){
        ...
        changeTopText(newText = "Civilians/Protesters issue ~"+d3.format(".0f")(matrix_raw.filter(function(d){return d.Source_Sector_General == "Civilians/Protesters";}).sum("events"))+"% of violence.",
            loc = 2/2, delayDisappear = 0, delayAppear = arcDelay[2]-1);
        /*Rebels*/
        changeTopText(newText = "Rebels are the second largest source, with ~"+d3.format(".0f")(matrix_raw.filter(function(d){return d.Source_Sector_General == "Rebels";}).sum("events"))+"%.",
            loc = 2/2, delayDisappear = arcDelay[3]-1, delayAppear = arcDelay[3]);
        /*ISIS, Al Qaeda / Jabhat al-Nusra, and Kurds*/
        changeTopText(newText = d3.format(".1f")(matrix_raw.filter(function(d){return (d.Source_Sector_General == "ISIS") || (d.Source_Sector_General == "Kurds") || (d.Source_Sector_General == "Al Qaeda / Jabhat al-Nusra") ;}).sum("events"))+"% comes from ISIS, Al Qaeda / Jabhat al-Nusra, and Kurds. Another "+d3.format(".1f")(matrix_raw.filter(function(d){return d.Source_Sector_General == "Unknown/Other";}).sum("events"))+"% comes from Others, which includes the private sector and the Media.",
            loc = 4/2, delayDisappear = (arcDelay[4]-1), delayAppear = arcDelay[4]);
        /*Unknown/Others*/
        changeTopText(newText = "\"Others\" include businesses and the Media, accounting for ~"+d3.format(".0f")(matrix_raw.filter(function(d){return d.Source_Sector_General == "Unknown/Other";}).sum("events"))+"%.",
        ...
        /*Foreign*/
        changeBottomText(newText = "Foreign actors account for ~"+d3.format(".0f")(matrix_raw.filter(function(d){return d.Source_Sector_General == "Foreign";}).sum("events"))+"%.",
    ...
    function Draw5(){
        ...
        /*Text*/
        changeTopText(newText = "On the bottom, touching the arc of Civilians / Protestors, the thickness of the cord spans ~"+d3.format(".0f")(matrix_raw.filter(function(d){return (d.Target_Sector_General == "Government/Military") && (d.Source_Sector_General == "Civilians/Protesters");}).sum("events"))+"%.",
            loc = 2, delayDisappear = 0, delayAppear = 1, finalText = true);
        ...
        // Tried to get an automated version of the arc (did not work)
        /*var helper = matrix_raw.filter(function(d){return d.Source_Sector_General == "Civilians/Protesters";}).sort(function(a, b){return b.events - a.events;});
        var lower_end = matrix_raw.filter(function(d){return d.Source_Sector_General == "Government/Military";}).sum("events");
        var upper_end = matrix_raw.filter(function(d){return d.Source_Sector_General == "Government/Military";}).sum("events");
        var hit = false;
        var i = 0;
        do{
            console.log(i);
            console.log(helper[i]);
            if (helper[i]["Target_Sector_General"]=="Government/Military"){
                upper_end += helper[i]["events"];
                hit = true;
            }
            else {
                lower_end += helper[i]["events"];
                upper_end += helper[i]["events"];
            }
            i++;
        }
        ...
        console.log(lower_end);
        console.log(upper_end);*/

        // Change here!
        var arcAJ = d3.svg.arc()
            .innerRadius(innerRadius)
            .outerRadius(outerRadius)
            .startAngle(2.815)// 2.83
            .endAngle(2.867); // 2.872
    ...
    function Draw7(){
        ...
        /*Text*/
        changeTopText(newText = "At the Government / Military side, the arc is much thicker, spanning "+d3.format(".0f")(matrix_raw.filter(function(d){return (d.Source_Sector_General == "Government/Military") && (d.Target_Sector_General == "Civilians/Protesters");}).sum("events"))+"% of total events.",
            loc = 2, delayDisappear = 0, delayAppear = 1);
        changeTopText(newText = "These "+d3.format(".0f")(matrix_raw.filter(function(d){return (d.Source_Sector_General == "Government/Military") && (d.Target_Sector_General == "Civilians/Protesters");}).sum("events"))+"% of events have the Government / Military as a source of violence. By following the chord we can see that they were targeted at Civilians / Protesters.",
    ...
    function Draw9(){
        ...
        chords.transition().duration(2000)
            .attr("opacity", function(d, i) {
                ...
                else if(d.source.index == 3 && d.target.index == 3) {return opacityValueBase;} // Rebels
    ...
    function Draw10(){
        ...
        changeTopText(newText = "For example, "+d3.format(".0f")(matrix_raw.filter(function(d){return (d.Source_Sector_General == "Rebels") && (d.Target_Sector_General == "Rebels");}).sum("events"))+"% of events that have Rebels as both a source and a target of violence.",
            loc = 2/2, delayDisappear = 0, delayAppear = 1);

        var helper = (matrix_raw.filter(function(d){return (d.Source_Sector_General == "Rebels") && (d.Target_Sector_General == "Rebels");}).sum("events") * 100.0 ) / matrix_raw.filter(function(d){return (d.Source_Sector_General == "Rebels");}).sum("events");
        changeTopText(newText = "This means that about "+d3.format(".0f")(helper)+"% of Rebels"+
                " violence events remained within that group.",
            loc = 2/2, delayDisappear = 9, delayAppear = 10, finalText = true);

        /*Show only the Rebels arc*/
    ...
    function Draw13(){
        ...
        changeTopText(newText = "Apart from the chord to Rebels, all other blue chords are narrow.",
            loc = 4/2, delayDisappear = 0, delayAppear = 1, finalText = false, xloc=0, w=200);
        changeTopText(newText = "This means that the main aggressor of the government are Rebels.",
