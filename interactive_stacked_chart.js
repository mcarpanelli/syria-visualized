...
// Choose Zero vs. Stacked
    var offsetSelect = d3.ui.select({
        base: visStacked,
        //before: "svgStacked",
        style: {position: "absolute", left: width - padding.right + 140 - move*0.25 + "px", top: 80 + "px", color: "black", opacity: 0.8},
        onchange: function() {
            if (dataSetSelect.value() == "dataSetSource") {
                var selectedDataSet = dataSetSource;
            } else if (dataSetSelect.value() =="dataSetTarget") {
...
// updateStacked: This is a function that takes in the data and updates the stacked bar based on the options
    ...
    function updateStacked(dataSet) {
        ...
        svgStacked.append("text")
            .text("DATA TYPE")
            .attr("transform", "translate("+ (350-move) +","+(0)+")rotate(0)")
            .attr("fill", "white");

        svgStacked.append("text")
            .text("CHART TYPE")
            .attr("transform", "translate("+ (465-move*1.25) +","+(0)+")rotate(0)")
            .attr("fill", "white");

        svgStacked.append("text")
            .text("Jan-Apr")
            .attr("transform", "translate("+ (267-move*0.83) +","+(335)+")rotate(0)")
            .attr("fill", "lightgray");
